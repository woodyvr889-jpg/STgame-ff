  ...
  function startGame(){
    ...
    timerInterval=setInterval(()=>{
      ...
      if(timeLeft<=0){
        ...
        storage.setItem("userStats",JSON.stringify(allStats));
      } else if(Math.random()<0.2){ spawnItem(); }
    },1000);
  }
...
function initHeaderButtons(){
  ...
  if(btnLogout) btnLogout.addEventListener("click",()=>{
    storage.removeItem("currentUser"); window.location.href="index.html";
  });
  ...
  if(btnGame) btnGame.addEventListener("click",()=>{
    if(isLockedNow()){ alert("Game locked!"); return; }
    window.location.href="game.html";
  });
  if(btnShop) btnShop.addEventListener("click",()=>{
    if(isLockedNow()){ alert("Shop locked!"); return; }
